...
Focus state handled by `.ring-input:focus` in `globals.css` — primary color ring, no extra classes needed.
...
All three border/shadow utilities are defined in `globals.css` and should be used instead of arbitrary `ring-*` or `border-*` values.
