...
sensitive actions and oversee the queue. A **super admin** provisions **teams** so each organization or environment can
have an isolated workspace with its own accounts.
...
**Sharing access:** Document the team slug and accounts for that workspace, e.g. “**Awano** — team `east-support` —
`support@demo.com` / `manager@demo.com`”.
...
- `@@unique([slug])` on `Team` (global slug for login URL: `/login?team=east-support`)
...
1. **Super** creates team `kanto-support`, seeds demo users, copies credentials for onboarding.
...
6. **Super** creates second team `tohoku-support` with same email patterns — isolated data.
...
## Example: sharing login details

```text
Awano — multi-team support desk
URL: https://...
Team: kanto-support
Accounts: support@kanto.com (Support), manager@kanto.com (Manager)
Login: /login · Password: oretachinomachida
```
...
Someone can: log in as Super → create team → seed users → log in as field agent in that team → create ticket → log in as
support in **same team** → resolve — and confirm **another team’s** support cannot see the ticket. Completed on the live
**Awano** URL in under 10 minutes.
