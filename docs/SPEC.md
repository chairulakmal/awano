...
| **Secrets**       | `AUTH_SECRET`, `AUTH_URL` set in Railway dashboard; never committed                                                                      |
...
| `trustHost: true` in `authConfig`                                         | Railway terminates TLS at its load balancer and forwards traffic to the app over plain HTTP internally. Without this flag, Auth.js does not trust the `x-forwarded-host` / `x-forwarded-proto` headers and cannot resolve the real public origin. When `redirectTo: "/"` is passed to `signIn`, Auth.js validates it against the inferred origin; if that check fails it falls back to `pages.signIn` (`/login`), dropping the `?team=` param and showing the wrong login form after a successful login. `trustHost: true` restores correct origin resolution and makes the post-login redirect work as intended on any reverse-proxied deployment. |
