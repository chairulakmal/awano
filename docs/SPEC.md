...
Awano is a multi-tenant support desk. External requesters (customers, recruiters, field agents) submit tickets; support staff triage and respond; managers oversee the queue and authorize sensitive actions. A super admin provisions isolated team workspaces.
...
Transition logic lives in `src/lib/tickets/fsm.ts`. Every accepted transition writes a `StatusEvent` row.
...
Login URL: `/login?team={slug}`. Team slug is resolved to `teamId` server-side before credential verification.
...
Server Actions validate input (Zod), read session, then delegate to service functions. Service functions enforce business rules and issue Prisma queries. No Prisma calls outside the service layer.
