...
export async function seedTickets(db: Db, ctx: TicketSeedContext) {
  ...
  await ticket(
    ...
    "IN_PROGRESS", "HIGH", ctx.supportAId
  );

  // WAITING_ON_REQUESTER · NORMAL — Indonesian worker, double dormitory deduction
  await ticket(
    db, "seed-ticket-a3", ctx.teamAId,
    ctx.customerAId, housingA.id,
    ...
    "WAITING_ON_REQUESTER", "NORMAL", ctx.supportAId
  );

  // ESCALATED · HIGH — Field agent, contract amendment dispute
  await ticket(
    db, "seed-ticket-a4", ctx.teamAId,
    ctx.agentAId, employA.id,
  ...
  await ticket(
    ...
    "RESOLVED", "NORMAL", ctx.supportAId
  );

  // OPEN · NORMAL — Recruiter, new-batch onboarding request
  await ticket(
    db, "seed-ticket-a6", ctx.teamAId,
    ctx.recruiterAId, employA.id,
  ...
  await ticket(
    db, "seed-ticket-a7", ctx.teamAId,
    rahmat.id, healthA.id,
  ...
  await ticket(
    db, "seed-ticket-a8", ctx.teamAId,
    ctx.agentAId, healthA.id,
  ...
  await ticket(
    db, "seed-ticket-a10", ctx.teamAId,
    ctx.customerAId, skillsA.id,
  ...
  await ticket(
    ...
    "IN_PROGRESS", "HIGH", ctx.supportBId
  );

  // ESCALATED · URGENT — Field agent, workplace injury and coerced negligence waiver
  await ticket(
    db, "seed-ticket-b3", ctx.teamBId,
    mahtwe.id, employB.id,
    ...
    "ESCALATED", "URGENT", ctx.supportBId
  );

  // WAITING_ON_REQUESTER · NORMAL — Company liaison, rental contract review
  await ticket(
    db, "seed-ticket-b4", ctx.teamBId,
    ctx.customerBId, housingB.id,
    ...
    "WAITING_ON_REQUESTER", "NORMAL", ctx.supportBId
  );

  // RESOLVED · LOW — Myanmar worker, driving licence conversion query
  await ticket(
    db, "seed-ticket-b5", ctx.teamBId,
    kyaw.id, dailyB.id,
  ...
  await ticket(
    ...
    "IN_PROGRESS", "NORMAL", ctx.supportBId
  );

  // CLOSED · NORMAL — Company liaison, pre-entry health check translation
  await ticket(
    db, "seed-ticket-b8", ctx.teamBId,
    ctx.customerBId, healthB.id,
  ...
  await Promise.all([
    ...
    ev(db, "seed-ev-a2-1", "seed-ticket-a2", ctx.supportAId, "IN_PROGRESS", "OPEN",
    ...
    ev(db, "seed-ev-a3-2", "seed-ticket-a3", ctx.supportAId, "WAITING_ON_REQUESTER", "IN_PROGRESS",
      "Asked requester to share a photo of the January payslip for verification"),

    // tA4: OPEN → IN_PROGRESS → ESCALATED
    ev(db, "seed-ev-a4-1", "seed-ticket-a4", ctx.supportAId, "IN_PROGRESS", "OPEN",
      "Reviewing contract amendment terms against Labour Standards Act"),
    ev(db, "seed-ev-a4-2", "seed-ticket-a4", ctx.managerAId, "ESCALATED", "IN_PROGRESS",
    ...
    ev(db, "seed-ev-a5-2", "seed-ticket-a5", ctx.supportAId, "RESOLVED", "IN_PROGRESS",
      "Translation arranged via certified agency; delivered to requester"),

    // tA7: OPEN → IN_PROGRESS
    ev(db, "seed-ev-a7-1", "seed-ticket-a7", ctx.supportAId, "IN_PROGRESS", "OPEN",
    ...
    ev(db, "seed-ev-a8-2", "seed-ticket-a8", ctx.supportAId, "RESOLVED", "IN_PROGRESS",
      "Bahasa Indonesia NHI guide sent to worker via field agent"),
    ev(db, "seed-ev-a8-3", "seed-ticket-a8", ctx.managerAId, "CLOSED", "RESOLVED",
    ...
    ev(db, "seed-ev-a10-2", "seed-ticket-a10", ctx.supportAId, "RESOLVED", "IN_PROGRESS",
      "Enrolled in N4 class starting next month; confirmation sent"),

    // tB2: OPEN → IN_PROGRESS
    ev(db, "seed-ev-b2-1", "seed-ticket-b2", ctx.supportBId, "IN_PROGRESS", "OPEN",
      "Reviewing apostille requirements with immigration specialist"),

    // tB3: OPEN → IN_PROGRESS → ESCALATED
    ev(db, "seed-ev-b3-1", "seed-ticket-b3", ctx.supportBId, "IN_PROGRESS", "OPEN",
      "Gathering details of the incident and the signed document"),
    ev(db, "seed-ev-b3-2", "seed-ticket-b3", ctx.managerBId, "ESCALATED", "IN_PROGRESS",
    ...
    ev(db, "seed-ev-b4-2", "seed-ticket-b4", ctx.supportBId, "WAITING_ON_REQUESTER", "IN_PROGRESS",
    ...
    ev(db, "seed-ev-b5-2", "seed-ticket-b5", ctx.supportBId, "RESOLVED", "IN_PROGRESS",
      "Confirmed Myanmar is on simplified conversion list; document checklist sent"),

    // tB7: OPEN → IN_PROGRESS
    ev(db, "seed-ev-b7-1", "seed-ticket-b7", ctx.supportBId, "IN_PROGRESS", "OPEN",
    ...
    ev(db, "seed-ev-b8-2", "seed-ticket-b8", ctx.supportBId, "RESOLVED", "IN_PROGRESS",
  ...
  await Promise.all([
    ...
    comment(db, "seed-comment-a2-1", "seed-ticket-a2", ctx.supportAId,
    ...
    comment(db, "seed-comment-a3-1", "seed-ticket-a3", ctx.supportAId,
    ...
    comment(db, "seed-comment-a3-2", "seed-ticket-a3", ctx.supportAId,
    ...
    comment(db, "seed-comment-a4-1", "seed-ticket-a4", ctx.managerAId,
    ...
    comment(db, "seed-comment-a5-1", "seed-ticket-a5", ctx.supportAId,
    ...
    comment(db, "seed-comment-a7-1", "seed-ticket-a7", ctx.supportAId,
    ...
    comment(db, "seed-comment-b3-1", "seed-ticket-b3", ctx.managerBId,
    ...
    comment(db, "seed-comment-b3-2", "seed-ticket-b3", ctx.supportBId,
    ...
    comment(db, "seed-comment-b6-1", "seed-ticket-b6", ctx.supportBId,
