...
import { loadMoreDeskTickets } from "./actions";
...
type DeskTicket = Awaited<ReturnType<typeof loadMoreDeskTickets>>["items"][number];

export function DeskTicketList({
  initialItems,
  initialCursor,
  view,
  query,
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-xl shadow-card bg-surface px-6 py-16 text-center">
        <p className="text-sm text-fg-subtle">No tickets here.</p>
      </div>
    );
  }
